...
export function createToolResultHandler(pi: any) {
	return async (event: any, ctx: Ctx) => {
		...
		if (mapped.tool_name === "Agent") patches.push(...buildSubagentPatches(await runHandlers(pi, "SubagentStop", extractSubagentType(event.toolName, event.input), { ...buildClaudeInputBase(ctx, "SubagentStop"), stop_hook_active: false, agent_id: event.toolCallId, agent_type: extractSubagentType(event.toolName, event.input), agent_transcript_path: undefined, last_assistant_message: textFromContent(event.content) }, ctx)));
