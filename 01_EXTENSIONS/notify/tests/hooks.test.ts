...
describe("createAgentEndHandler", () => {
	...
	it("prefers the Korean summary as the notification title", async () => {
		...
		expect(notify).toHaveBeenCalledWith("로그인 문구 수정 완료", "");
	});
	...
	it("strips a leading session title from the summary", async () => {
		...
		expect(notify).toHaveBeenCalledWith("서브에이전트 두 명이 모두 가위를 내 무승부로 끝났어", "");
	});

	it("falls back to the local Korean title", async () => {
