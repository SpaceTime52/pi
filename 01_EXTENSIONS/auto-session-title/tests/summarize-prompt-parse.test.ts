...
describe("buildOverviewPrompt", () => {
	it("tells the model to produce scan-friendly current-state bullets for future recall", () => {
		...
		expect(prompt).toContain("Write SUMMARY as 1-7 short `- ` bullets when durable state exists.");
...
describe("parseOverviewResponse", () => {
	...
	it("caps fallback sentence splitting at seven summary lines", () => {
		expect(parseOverviewResponse("TITLE: 긴 요약\nSUMMARY: 첫 문장은 현재 목표와 방향을 길게 설명하는 문장이다. 둘째 문장은 이미 끝낸 검증과 확인 결과를 길게 설명하는 문장이다. 셋째 문장은 남아 있는 제약과 주의사항을 길게 설명하는 문장이다. 넷째 문장은 다음 단계 계획을 길게 설명하는 문장이다. 다섯째 문장은 추가 맥락 5다. 여섯째 문장은 추가 맥락 6이다. 일곱째 문장은 마지막 줄에 합쳐질 추가 맥락이다. 여덟째 문장도 마지막 줄에 함께 붙어야 하는 추가 맥락이다.")).toEqual({ title: "긴 요약", summary: ["첫 문장은 현재 목표와 방향을 길게 설명하는 문장이다.", "둘째 문장은 이미 끝낸 검증과 확인 결과를 길게 설명하는 문장이다.", "셋째 문장은 남아 있는 제약과 주의사항을 길게 설명하는 문장이다.", "넷째 문장은 다음 단계 계획을 길게 설명하는 문장이다.", "다섯째 문장은 추가 맥락 5다.", "여섯째 문장은 추가 맥락 6이다.", "일곱째 문장은 마지막 줄에 합쳐질 추가 맥락이다. 여덟째 문장도 마지막 줄에 함께 붙어야 하는 추가 맥락이다."] });
	});
