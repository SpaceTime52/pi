...
const entry = (id: string, title: string, ...summary: string[]) => ({ type: "custom", id, customType: "auto-session-title.overview", data: { title, summary } });
const branch = (id: string, title: string, ...summary: string[]) => [entry(id, title, ...summary)];
...
describe("overview restoration overlay lifecycle", () => {
	...
	it("replaces the old overlay when the session id changes", () => {
		const firstCtx = stubContext(branch("1", "첫 세션", "현재 상태를 짧게 표시함"));
		restoreOverview(stubRuntime(), firstCtx);
		const secondBranch = branch("2", "둘째 세션", "다음 상태로 전환함");
		const secondCtx = stubContext(secondBranch, { sessionManager: { ...stubContext(secondBranch).sessionManager, getSessionId: () => "session-2" } });
	...
	it("recreates the overlay when terminal width changes so right anchoring stays stable", () => {
		const ctx = stubContext(branch("1", "현재 세션", "현재 상태를 짧게 표시함"));
	...
	it("clears pending overlay state when opening the overlay later rejects", async () => {
		...
		await Promise.resolve();
		const nextCtx = stubContext(branch("2", "다음 세션", "다음 상태로 전환함"));
