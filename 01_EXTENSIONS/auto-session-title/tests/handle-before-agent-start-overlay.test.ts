...
describe("overview restoration overlay lifecycle", () => {
	...
	it("replaces the old overlay when the session id changes", () => {
		const firstCtx = stubContext([{ type: "custom", id: "1", customType: "auto-session-title.overview", data: { title: "첫 세션", summary: ["Goal: A"] } }]);
		restoreOverview(stubRuntime(), firstCtx);
		const secondCtx = stubContext([{ type: "custom", id: "2", customType: "auto-session-title.overview", data: { title: "둘째 세션", summary: ["Goal: B"] } }], { sessionManager: { ...stubContext().sessionManager, getSessionId: () => "session-2" } });
	...
	it("clears pending overlay state when opening the overlay later rejects", async () => {
		const ctx = stubContext([{ type: "custom", id: "1", customType: "auto-session-title.overview", data: { title: "현재 세션", summary: ["Goal: A"] } }]);
		...
		await Promise.resolve();
		const nextCtx = stubContext([{ type: "custom", id: "2", customType: "auto-session-title.overview", data: { title: "다음 세션", summary: ["Goal: B"] } }]);
