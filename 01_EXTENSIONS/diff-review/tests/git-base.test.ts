...
import { findReviewBase, getRepoRoot, hasHead, listCommits } from "../src/git-base.ts";

function createApi(stdoutByCommand: Record<string, { code: number; stdout: string }>) {
	return {
		exec: async (command: string, args: string[]) => {
			const result = stdoutByCommand[[command, ...args].join(" ")];
			return result ? { ...result, stderr: "" } : { code: 1, stdout: "", stderr: "" };
		},
	};
}
...
describe("git base helpers", () => {
	...
	it("finds a merge base and handles missing repos", async () => {
		const api = createApi({
	...
	it("returns null when no review base exists", async () => {
		const api = createApi({ "git branch --show-current": { code: 0, stdout: "HEAD\n" } });
		await expect(findReviewBase(api, "/repo")).resolves.toBeNull();
		await expect(hasHead(createApi({}), "/repo")).resolves.toBe(false);
		await expect(findReviewBase(createApi({ "git branch --show-current": { code: 0, stdout: "\n" } }), "/repo")).resolves.toBeNull();
	});
});
