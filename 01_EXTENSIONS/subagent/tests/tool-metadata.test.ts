...
describe("createTool metadata and listings", () => {
	...
	it("lists active runs, empty runs, and history", async () => {
		...
		expect((await exec("runs")).content[0].text).toContain("Active (1)");
		resetStore();
		expect((await exec("runs")).content[0].text).toBe("No runs");
	...
	it("renders details for missing and existing history", async () => {
		expect((await exec("detail 999")).content[0].text).toContain("not found");
		addToHistory({ id: 5, agent: "scout", output: "result text" });
		expect((await exec("detail 5")).content[0].text).toContain("result text");
		...
		expect((await exec("detail 7")).content[0].text).toContain("→ Bash");
		expect((await exec("runs")).details.isError).toBe(false);
	});
});
