...
export function createSessionRunner(sessFile, ctx) {
    return async (agent, task) => {
        const id = nextId();
        const { cmd, base } = getPiCommand(process.execPath, process.argv[1], existsSync);
        const args = buildArgs({ base, model: agent.model, tools: agent.tools, systemPromptPath: "", task, sessionPath: sessFile });
