...
export function createClaudeEditTool(cwd: string) {
	...
	return defineTool({
		...
		renderResult(result: EditResult, { expanded, isPartial }: RenderOptions, theme: Theme) {
			...
			if (!result.details?.diff) return new Text(theme.fg("success", "applied"), 0, 0);
			const diffLines = result.details.diff.split("\n");
			let text = theme.fg("success", `+${diffLines.filter((line) => line.startsWith("+") && !line.startsWith("+++")).length}`);
			text += theme.fg("dim", " · ") + theme.fg("error", `-${diffLines.filter((line) => line.startsWith("-") && !line.startsWith("---")).length}`);
